...
const CommitWrap = require("./CommitWrap");
...
function getRepoPath (git) {
	return path.resolve(git.options.gitWorkTree, git.options.gitDir);
}

class Git extends Base {
	setRepository (repository) {
		repository.setWorkdir(this.options.gitWorkTree, 0);
	...
	async init () {
		...
		await this.config("core.bare", "false");
		return repository;
	}
	...
	async config (key, value) {
		...
		return value ? config.setString(key, value) : config.getStringBuf(key);
	}
	async commit (options) {
		const repository = await this.repository();
		const userName = await this.config("user.name");
		const userEmail = await this.config("user.email");
		const time = new Date();
		const signature = Signature.create(userName, userEmail, time.valueOf() / 1000, -time.getTimezoneOffset());
		const commit = await repository.createCommitOnHead([], signature, signature, options.message || "");
		delete this.getHeadCommit;
		return commit;
	}
	async lsFiles () {
		const index = await this.index();
