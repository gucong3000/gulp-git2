...
class Git {
	constructor (options = {}) {
		...
		this.options = {
			gitWorkTree: gitWorkTree ? path.resolve(gitWorkTree) : process.cwd(),
			gitDir: options.gitDir || process.env.GIT_DIR || ".git",
			config: options.config || {},
			debug: options.debug || false,
			publicKey: options.publicKey || "~/.ssh/id_rsa.pub",
			privateKey: options.privateKey || "~/.ssh/id_rsa",
		};
	}
	blame (...options) {
		if (isVinyl(options)) {
	...
	relative (file) {
		let relative = path.relative(this.options.gitWorkTree, file);
		if (path.sep === "\\") {
